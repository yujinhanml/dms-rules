...
def generate_spiral_indices(image_size, patch_size):
    """
    Generate spiral order indices for tokens in the image.
    """
    grid_size = image_size // patch_size
    indices = np.arange(grid_size**2).reshape(grid_size, grid_size)
    spiral_order = []
    
    while indices.size:
        # Take the first row
        spiral_order.extend(indices[0])
        indices = indices[1:]
        if indices.size == 0:
            break
        # Take the last column
        spiral_order.extend(indices[:, -1])
        indices = np.delete(indices, -1, axis=1)
        if indices.size == 0:
            break
        # Take the last row (reversed)
        spiral_order.extend(indices[-1][::-1])
        indices = indices[:-1]
        if indices.size == 0:
            break
        # Take the first column (reversed)
        spiral_order.extend(indices[:, 0][::-1])
...
class AutoregressiveDiffusion(Module):
    ...
    def sample(
        ...
        batch_size = 1,
        prompt  = None,
        spiral_indices = None  # New parameter for spiral indices
    ):
        self.eval()
        ...
        for i in tqdm(range(self.max_seq_len - out.shape[1]), desc = 'tokens'):
            ...
            # If using spiral order, rearrange tokens back to the correct position
            if spiral_indices is not None:
                out_spiral = torch.zeros_like(out)
                out_spiral[:, spiral_indices[i]] = denoised_pred
                out = out_spiral
            else:
    ...
    def forward(
        ...
        # print(f"Input shape: batch={b}, seq_len={seq_len}, dim={dim}")
        # print(f"Expected dim_input: {self.dim_input}")
        # print(f"Expected max_seq_len: {self.max_seq_len}")
...
class ImageAutoregressiveDiffusion(Module):
    def __init__(
        ...
        *,
        ...
        channels = 3,
        model: dict = dict(),
        use_spiral_order = False  # New parameter
    ):
        super().__init__()
        ...
        self.use_spiral_order = use_spiral_order
        ...
        # Generate spiral order indices if required
        if self.use_spiral_order:
            self.spiral_indices = generate_spiral_indices(image_size, patch_size)

    def rearrange_tokens(self, tokens, forward=True):
        """
        Rearrange tokens to or from spiral order.
        If `forward=True`, convert to spiral order; otherwise revert to grid order.
        """
        if not self.use_spiral_order:
            return tokens

        if forward:
            return tokens[:, self.spiral_indices]
        else:
            reverse_indices = np.argsort(self.spiral_indices)
            return tokens[:, reverse_indices]

    def sample(self, batch_size = 1):
        spiral_indices = self.spiral_indices if self.use_spiral_order else None
        tokens = self.model.sample(batch_size = batch_size)
        
        # Revert spiral order if necessary
        tokens = self.rearrange_tokens(tokens, forward=False)
    ...
    def forward(self, images):
        ...
        # Convert to spiral order if necessary
        tokens = self.rearrange_tokens(tokens, forward=True)
