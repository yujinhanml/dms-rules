...
def generate(args):
    ...
    result_path = os.path.join(args.result_path,f'epoch_{gen_epoch}_{str(time.time())}')
    ...
    if conditional:
        ...
        x = diffusion.sample(model=model, n=num_images, phase = args.phase,result_path = result_path, save_intermediate = args.save_intermediate, intermediate_steps = args.intermediate_steps)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--intermediate_steps', type=str, help='Comma separated list of steps to save, e.g. "100,950,900"')

    args = parser.parse_args()
    if args.intermediate_steps:
        args.intermediate_steps = [int(step) for step in args.intermediate_steps.split(',')]
    else:
