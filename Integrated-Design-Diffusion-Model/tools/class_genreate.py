...
def generate(args):
    ...
    result_path = os.path.join(args.result_path,f'epoch_{gen_epoch}_cfg{args.cfg_scale}_class{os.path.basename(args.classifier_path)}_{str(time.time())}')
    ...
    if conditional:
        ...
        x = diffusion.contrastive_sample(model=model, n=num_images, labels=y, classifier=classifier,cond_fn=cond_fn, conditional = True)
    else:
        class_name = args.class_name
        if class_name == -1:
            y = torch.arange(num_classes).long().to(device)
        ...
        x = diffusion.contrastive_sample(model=model, n=num_images, labels=y, classifier=classifier,cond_fn=cond_fn, conditional = False)
