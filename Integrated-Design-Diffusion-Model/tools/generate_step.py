...
def generate(args):
    ...
    result_path = os.path.join(args.result_path,f'step_epoch_{gen_epoch}_{str(time.time())}')
    ...
    # save_path = os.path.join(result_path, f"{generate_name}.{image_format}")
    # Initialize model
    if conditional:
        ...
        x = diffusion.sample_step(model=model, n=num_images,save_intermediate = 10,output_dir = result_path,image_size = new_image_size)
